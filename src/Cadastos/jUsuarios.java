...
public class jUsuarios extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        mFuncao.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "recepção", "medico", "gerencia" }));
        ...
        mTelefone.setText("99 9999-9999");
    ...
    public void ReadFields() {
        ...
        try {
            ...
            if (!tfuncao.trim().toLowerCase().substring(0, 3).equalsIgnoreCase("med")) {
                lbCTPS_CRM.setText("CTPS:");
                lbUFCRM.setVisible(false);
                mUfCRM.setVisible(false);
                mCTPS_CRM.setSize(86, 28);
                ...
                String[][] mdcampos = conn.LerCamposTabela(
                        new String[] {"md_codigo", "md_categoria", "md_crm", "md_crmuf"}, "medicos", "md_cpf = '" + tcpf + "'");
                try {cdMedico = mdcampos[0][3];} catch (Exception e) {}
                try {tctps = mdcampos[2][3];} catch (Exception e) {}
                try {tcrmuf = mdcampos[3][3];} catch (Exception e) {}
                try {tespecialidade = mdcampos[1][3];} catch (Exception e) {}
            }
        ...
        if (mFuncao.getSelectedItem().toString().substring(0, 3).equalsIgnoreCase("rec") || mFuncao.getSelectedItem().toString().substring(0, 3).equalsIgnoreCase("ger")) {
            lbCTPS_CRM.setText("CTPS:");
            lbUFCRM.setVisible(false);
            mUfCRM.setVisible(false);
            mCTPS_CRM.setSize(86, 28);
    ...
    private void SaveFields(boolean save) {
        ...
        if (save) {
            ...
            try {
                ...
                // tabela do médico
                sql = "UPDATE medicos " +
                      "SET md_codigo=?, md_nome=?, md_categoria=?, md_cpf=?, " +
                      "md_identidade=?, md_crm=?, md_crmuf=?, " +
                      "md_endereco=?, md_bairro=?, md_cidade=?, md_estado=?, " + 
                      "md_cep=?, md_telefone=? WHERE md_codigo=?;";
                stm = conn.conn.prepareStatement(sql);
                // coloca os campos
                stm.setInt(1, Integer.valueOf(tcdmed));
                stm.setString(2, tnome);
                stm.setString(3, tespecialidade);
                stm.setString(4, tcpf);
                stm.setString(5, trg);
                ...
                stm.setString(12, tcep);
                stm.setString(13, ttelefone);
                stm.setInt(14, Integer.valueOf(tcdmed));
                stm.execute();
                stm.close();

                // tabela do medconv
                sql = "UPDATE medconv " +
                      "SET mc_crm=?, mc_plano=?, mc_segunda=?, mc_terca=?, " +
                      "mc_quarta=?, mc_quinta=?, mc_sexta=?, " +
                      "mc_sabado=?, mc_domingo=? WHERE mc_medcod=?;";
                stm = conn.conn.prepareStatement(sql);
                // coloca os campos
                stm.setString(1, tctps);
                stm.setString(2, "PARTICULAR");
